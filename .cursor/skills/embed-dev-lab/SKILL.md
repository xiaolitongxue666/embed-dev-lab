...
| 工程 | 实现 | 串口 | 产物 |
|------|------|------|------|
| `f103-manual-reg` | 手写寄存器路线（无 CMSIS/HAL）；`src/app` `board` `periph` `driver` | `printf` + DMA TX/RX + IDLE + `syscalls.c`；ADC1 PA0 DMA；TIM2 风扇；SPI1 BMP280（LSM6 驱动保留）；I2C1 SH1106 时钟+右下温度（页 DMA1 CH6） | `projects/f103-manual-reg/build/f103-manual-reg.elf` |
