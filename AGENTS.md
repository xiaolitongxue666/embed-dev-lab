...
- **f103-manual-reg**：手写外设寄存器完成功能；不链接官方 CMSIS Device / HAL；串口 `printf` + `syscalls.c` → `_write` → DMA TX；RX 为 DMA + 空闲中断 + `USART1_ProcessRx`；ADC1 PA0 旋钮单次采样；SPI1 LSM6DS3；I2C1 PB6/PB7 SH1106（写地址 `0x78`，中央 `HH:MM:SS`）；SysTick 1 ms；时钟在 `SystemInit`（进 `main` 前）升至 72 MHz
