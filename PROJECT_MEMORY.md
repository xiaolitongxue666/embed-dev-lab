...
3. **代理**：`scripts/lib/proxy.sh` 默认 `http://127.0.0.1:7890`；bootstrap/install-extensions/fetch-st-docs 会应用。
...
14. **ST PDF 下载**：官网**无需注册**；CLI curl 对 st.com 常超时/SSL 中断，可浏览器直链另存为 `pdf/` 后 `--verify-only`。
15. **RM0008 版本**：本地可能为 Keil 镜像 Rev 9（995 页）而非 ST Rev 21（1136 页）；寄存器一致，topic MD 页码以本地 PDF 为准。
16. **用户文档**：`doc/` 中文；入口 `doc/README.md`；脚本说明 `doc/scripts-reference.md`。
17. **install_packet**：ST-Link WinUSB 驱动 + F103 核心板 MDK 例程（gitignore 大目录）；与 `doc/reference/` 官方 PDF 分工不同。
18. **USB / ST-Link**：绿联 Hub 下 ST-Link V2 (`0483:3748`) 可正常枚举；SWD 四线 SWDIO/SWCLK/GND/3.3V。
19. **clangd**：build/bootstrap 后 `setup-clangd.sh` 同步根 `compile_commands.json`。
20. **调试**：Cursor Run →「F103 Probe-rs Debug」；`.vscode/launch.json` 已配置。
21. **f103-blink 注释**：源码中文；终端/CLI 输出保持英文。
22. **`.gitignore`**：`install_packet/STM32F103C8T6核心板/`、`doc/reference/stm32f103/pdf/*.pdf` 等大/本地文件。
23. **Cursor 终端**：工作区 `.vscode/settings.json` 声明 `defaultProfile: Git Bash`。
24. **OpenOCD**：bootstrap 可能因 GitHub 下载失败未安装；不影响 probe-rs 烧录。
25. **实机验证**：ST-Link V2 + SWD + 3.3V，PC13 LED 正常闪烁（Backup 域 DBP 修复后）。
