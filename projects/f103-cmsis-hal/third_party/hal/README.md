...
> **注意**：本目录 **不是** 完整 HAL 驱动仓库。完整 HAL 在 `vendor-pack/stm32f1xx-hal-driver`（**git submodule**，`v1.1.8`）。fetch 策略：**Inc 全拷**（`stm32f1xx_hal*.h` / `ll*.h` / `Legacy/`），**Src 仅拷本 demo 需要的 9 个 .c**（含 `stm32f1xx_hal_uart.c`）并由 CMake 链入固件；其余模块仅有头文件、无 `.c`，不占 Flash。
...
- **`Src/`**：仅拷贝本 demo **链接** 的 9 个 `.c`（见根 [`README.md`](../README.md)），含 UART。

未链入的 HAL 模块（SPI、TIM 等）头文件在 `Inc/` 中存在，但无对应 `.c`，**不会增加 Flash 占用**。
