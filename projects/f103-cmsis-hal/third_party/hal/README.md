...
> **注意**：本目录 **不是** 完整 HAL 驱动仓库。完整 HAL 在 `vendor-pack/stm32f1xx-hal-driver`（**git submodule**，`v1.1.8`）。fetch 策略：**Inc 全拷**（`stm32f1xx_hal*.h` / `ll*.h` / `Legacy/`），**Src 仅拷本 demo 需要的 16 个 .c**（含 `stm32f1xx_hal_uart.c`）并由 CMake 链入固件；其余模块仅有头文件、无 `.c`，不占 Flash。
...
- **`Src/`**：仅拷贝本 demo **链接** 的 16 个 `.c`（见根 [`README.md`](../README.md)），含 UART。

未链入的 HAL 模块头文件在 `Inc/` 中存在，但无对应 `.c`，**不会增加 Flash 占用**。
...
usart.c
  ...
  HAL_UART_MspInit（stm32f1xx_hal_msp.c）→ PA9/PA10

spi.c / i2c.c / adc.c / tim.c
  HAL_SPI_* / HAL_I2C_* / HAL_ADC_Start_DMA / HAL_TIM_PWM_*
...
| `Src/*.c`（上述 16 个） | **是** | 进入 `.text` / `.rodata` |

## 注释约定

- ST 原版 `@brief` / `@verbatim` **保持英文**；fetch 覆盖后无需手工翻译。
- 每个参与链接的 `Src/*.c` 及 `Inc/stm32f1xx_hal.h` 顶部的 **embed-dev-lab 中文块** 说明在本工程中的用途；由 `scripts/lib/apply-f103-cmsis-hal-comments.sh` 在 **copy_hal_minimal 之后** 注入。
