...
3) Dual tracks: f103-manual-reg (bare regs) vs f103-cmsis-hal (CMSIS+HAL Cube-style). Align external behavior unless user scopes one track. 2026-09-21 both tracks: software timers (OLED 50ms / LED 1s), four-field OLED `00:00:00:00`, LOG_* `[HH:MM:SS][I]` + ANSI.
...
11) SH1106 (both tracks 2026-09-21): DrawClock(h,m,s,centi) draws `00:00:00:00` at 8×16 (11 glyphs, x=20 y=24); 2x HH:MM:SS does not fit 128 cols. Temp still y=48. Refresh 50ms (TIMER_EVT_OLED_CLOCK); temp cached, BMP280 read on 1s slot. After Init: manual-reg SetMs(0); cmsis-hal SysTime_Reset() (does not touch uwTick).
...
13) timer_event: 4 slots; period_ms==0 skip; TakeFlag read-clear. OLED 50ms periodic; LED/log 1000ms periodic. main has no delay(0x7FFF)/led_phase. manual-reg: periph + cpsid. cmsis-hal: flat src/ + CMSIS __disable_irq; SysTick_Handler = HAL_IncTick then OnTick.
14) LOG_* `[HH:MM:SS][I]` second-only; E red / W yel / I grn / D cyan ANSI; LOG_COMPILE_LEVEL default DEBUG; LOG_COLOR=1. One vsnprintf + one TX per line. 1s DEBUG is **one** line (LED/KEY/knob/clock[/temp]). Never log in ISR. SecureCRT needs ANSI Color. manual-reg: one printf. cmsis-hal: one HAL_UART_Transmit (no syscalls).
...
16) cmsis-hal: fetch-cmsis.sh then fetch-f103-cmsis-hal-deps.sh; HAL Src **16** .c; src/log.c + src/timer_event.c; comments Chinese in src/startup/linker; CLI English. Clock in main SystemClock_Config.
...
21) 2026-09-21 both tracks flash OK COM8 1500000: `[00:00:01][D] LED on  KEY high  knob raw=…  clock 00:00:01:00` ~1s cyan. Boot `[I]` green. OLED four-field running. BMP280 temp often absent this bench (init fail → 0.00C).
