...
| 轨 | 约上限 | 适合 | 不适合 |
|----|--------|------|--------|
| ST-Link 3.3V | ≈150 mA | OLED、LSM6DS3、BMP280、BH1750 等小电流传感器 | 继电器、舵机、大功率 LED、**JY003 风扇电机** |
...
### 场景 3：JY003 风扇（电机独立电源）

| 模块脚（以丝印为准） | 接到 |
|----------------------|------|
| PWM / IN / SIG | 蓝板 **PA1**（TIM2_CH2，3.3 V 逻辑；阶段 4） |
| 电机 VCC / VIN | **模块自带 12V 或 5V 电源**，禁止接面包板 3.3V / MCU GPIO |
| GND | 面包板 GND 轨（与 MCU / ST-Link / CH341 共地） |

风扇电流远超 ST-Link 3.3V ≈150 mA。只共地、不共 3.3V。PWM 脚勿接到 PA0（旋钮）。
