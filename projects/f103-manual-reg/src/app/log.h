/**
 * @file    log.h
 * @brief   分级日志：时间戳 + 级别字母，经 printf → USART1
 *
 * 行格式：[HH:MM:SS][I] message（时间只到秒）
 * 禁止在 ISR 中调用。SysTick_Init 之前时间为 00:00:00。
 *
 * LOG_COMPILE_LEVEL 以上的级别直接丢弃。
 * LOG_COLOR=1 时 ANSI 着色（SecureCRT 须开 ANSI Color）。
 *
 * @see     log.c
 */
