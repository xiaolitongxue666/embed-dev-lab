...
10. **CMSIS 与本仓库**：专题 `doc/learn/cmsis-overview.md`；向量表/NVIC 专题 `doc/learn/interrupt-vector-table-and-nvic.md`；f103-blink startup **仅前 16 项内核异常**（外设 IRQ 精简省略），不链接官方 CMSIS/HAL（兼容≠调用），遵循 `SystemInit`/向量表规范；CubeF1 对照 `.../Templates/gcc/startup_stm32f103xb.s`。
...
15. **用户文档**：`doc/` 中文；入口 `doc/README.md`；脚本 `doc/scripts-reference.md`；学习笔记 `doc/learn/`（bootstrap、编译流程、map 文件、CMSIS、中断/NVIC、DS/RM 分工）。
16. **USB / ST-Link**：绿联 Hub 下 ST-Link V2 (`0483:3748`) 可正常枚举；SWD 四线 SWDIO/SWCLK/GND/3.3V。
17. **clangd**：build/bootstrap 后 `setup-clangd.sh` 同步根 `compile_commands.json`。
18. **调试**：Cursor Run →「F103 Probe-rs Debug」；`.vscode/launch.json` 已配置。
19. **f103-blink 注释**：源码中文；终端/CLI 输出保持英文。
20. **`.gitignore`**：`vendor-pack/STM32CubeF1/`（除 README）、核心板目录、PDF、`.tools/` 忽略；`STLink/USBDriver` 可提交。
21. **MCP/Skill**：`install-mcp-skills.sh` 构建 embedded-debugger-mcp → `.cursor/mcp.json` + `skills/embed-dev-lab`；`--global` 合并多 Agent；Codex 无 MCP。
22. **实机验证**（2026-06-11）：ST-Link V2 + SWD + 3.3V，PC13 LED 闪烁（Backup 域 DBP 修复后）；embedded-debugger MCP（list/connect/reset/read_memory）通过；`probe-rs download` + `reset` 约 0.3s；Cursor Agent 需 MCP Connected + 重载/新对话。
23. **Cursor 终端**：工作区 `.vscode/settings.json` 声明 `defaultProfile: Git Bash`。
